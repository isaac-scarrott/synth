---
name: port-working-html
description: Port a change that has landed in working.html into the native SwiftUI Synth app (app/Sources/Synth/) and verify it by driving the real app. Fans out sub-agents in isolated git worktrees to implement independent slices concurrently, then integrates, builds, and drives the app to prove it works. Use when the user says the design in working.html is ready and wants it implemented/built into the native app, when porting a working.html change to the app, or invokes /port-working-html.
---

# Port working.html → native app
...
1. **Scope the change (intent + diff + audit).** In the main tree, gather all three:
   ...
   - **Diff** — `git log -p -- working.html` since the app last synced (find the relevant commits).
   - **Audit** — spawn parallel read-only sub-agents (Explore) to compare `working.html` against the
     current app and list gaps.
   Serve the reference: `python3 -m http.server 8912` (repo root) → `http://localhost:8912/working.html`.
...
3. **Implement (concurrent across slices).** Spawn one sub-agent per slice, each told to work **only
   ...
   its branch. Give each sub-agent the intent, the relevant `working.html` markup/CSS, and TESTING.md.
...
5. **Verify the integrated app.** Drive the *real* built app and screenshot it, comparing against
   `working.html` served at :8912 — see [TESTING.md](TESTING.md). For anything nontrivial, also spawn
...
- [ ] The change is visible in a screenshot of the running app and matches working.html.
