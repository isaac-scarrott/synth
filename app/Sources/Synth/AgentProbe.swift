...
enum AgentProbe {
    ...
    /// a string the user typed, and `--version` is the only argument it is ever given.
    private static func run(_ command: String, _ signatures: [Signature]) -> AgentProbeResult {
        ...
        let quoted = shellQuoteAgentArg(command)
        proc.arguments = ["-l", "-c",
        ...
        guard let line, !line.isEmpty else {
            ...
            return AgentProbeResult(state: .unrecognised, version: nil)
        }
        let hay = line.lowercased()
        if let match = signatures.first(where: { s in s.markers.contains { hay.contains($0) } }) {
            return AgentProbeResult(state: .recognised(match.id), version: line)
        }
        return AgentProbeResult(state: .unrecognised, version: line)
    }
