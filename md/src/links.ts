...
/// Resolve a link href as written in `fromPath`'s document. A `#fragment` rides along on a
/// document target — same-file or cross-file — so the caller can finish the jump once the
...
export function resolveLink(href: string, fromPath: string): LinkTarget {
  ...
  if (trimmed.startsWith("#")) return document(fromPath, trimmed.slice(1))
  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(trimmed)) {
    ...
    if (trimmed.toLowerCase().startsWith("file://")) {
      const [bare, fragment] = trimmed.slice("file://".length).replace(/^[^/]*/, "").split("#")
      const path = decodeURIComponent(bare)
      if (MARKDOWN.test(path) && existsSync(path)) return document(path, fragment)
    }
    return { kind: "external", url: trimmed }
  }

  const [withoutFragment, fragment] = trimmed.split("#")
  if (withoutFragment === "") return document(fromPath, fragment)
...
function document(path: string, fragment?: string): LinkTarget {
  return fragment ? { kind: "document", path, fragment } : { kind: "document", path }
}

/// The anchor GitHub mints for a heading — which is what an author writing `[…](#section)`
/// is quoting. Lowercase, punctuation dropped, whitespace to hyphens. Hyphen runs collapse,
/// which GitHub's own slugs do not do — but BOTH sides of the comparison come through here,
/// so a GitHub-minted `#v20--plan` and the heading it names still meet in the middle, and a
/// hand-typed single hyphen matches too.
