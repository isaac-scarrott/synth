...
| **Agent** | Which coding agent a session is running. Claude Code, OpenCode, Antigravity, or one you named yourself. | A configuration inside that program. OpenCode's agents are Build and Plan; Antigravity's are personas you switch with `/agents`; Claude Code uses the word for subagents and teammates as well. All of those live inside one Synth agent. |
...
| **Archive** | Puts a branch row away. The folder is untouched and the row comes back from `⌘K` whenever you want it. |
